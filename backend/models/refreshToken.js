...
// Ensure your table columns: tokenID (PK, AUTO_INCREMENT), userID, token_hash (UNIQUE), user_agent, ip, expires_at, revoked (BOOL DEFAULT 0)
async function storeRefreshToken(
  userId,
  token_hash,
  user_agent,
  ...
  const sql = `
    INSERT INTO refreshtokens (userID, token_hash, user_agent, ip, expires_at)
    VALUES (?, ?, ?, ?, ?)
  `;
  return pool.query(sql, [userId, token_hash, user_agent, ip, expires_at]);
}
...
async function findRefreshToken(token_hash, userId) {
  const sql = `SELECT * FROM refreshtokens WHERE token_hash = ? AND userID = ?`;
  const [rows] = await pool.query(sql, [token_hash, userId]);
  return rows[0] || null;
}
